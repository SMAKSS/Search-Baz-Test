...
function Search() {
    ...
    const searchURL = "https://bazlia.com/api/isearch/getProductListByKeyword"
    const imageURL = "https://media.bazlia.com/thumbnail"
    ...
    const search = (event) => {
        event.persist()
        setState((prevStates) => ({ ...prevStates, searchText: event?.target?.value }))
    }
     ...
     */
    const apiCall = ({ page = 1, perPage = 8, sort = "asc", text = "", width = 500, height = 625 }) => {
        fetch(`${searchURL}?page=${page}&perPage=${perPage}&sort[price][eq]=${sort}&keyword=${text}`)
            .then((response) => response.ok
                ? response.json()
                : response.text().then((text) => {
                    setState((prevStates) => ({ ...prevStates, error: true, errorContent: text }))
                }))
            .then(async (data) => {
                const list = [...data?.content?.data?.list]
                const length = list.length
                for (let i = 0; i < length; ++i) {
                    await fetch(`${imageURL}?width=${width}&height=${height}&file=${list[i].sourceMedia[0].url.split('.com/')[1]}`, { mode: 'no-cors' })
     ...
     */
    const clearSearch = () => {
        setState((prevStates) => ({ ...prevStates, searchText: "", requestData: null }))
    }
    ...
    const searchBarController = (event) => {
        event.target.classList.contains("container")
            ...
            : !searchBar.current.classList.contains("active") && searchBar.current.classList.add("active")
    }
     ...
     */
    useEffect(() => {
        !state.searchText && setState((prevStates) => ({ ...prevStates, requestData: null }))
        const timeOut = state.searchText ? setTimeout(() => apiCall({ text: state.searchText }), timeOutDelay) : null
    ...
    useEffect(() => {
        if (state.error) {
            ...
            const timeOut = setTimeout(() => snackBar.current.classList.remove("show"), 3000)
            return () => clearTimeout(timeOut)
        }
    ...
    return (
        <div className="container" onClick={(event) => searchBarController(event)}>
            <div ref={searchBar} className={state.searchText ? "active search-bar" : "search-bar"}>
                <span className={state.searchText ? "clear-search" : "d-none"} onClick={() => clearSearch()}></span>
                <input type="text" onChange={(event) => search(event)} value={state.searchText}
                    placeholder={`${general[local.local].search}...`} aria-label={'search'} />
                                ...
                                state.requestData.map((result, index) => {
                                    return (
                                        <li className="result-row" key={index}>
                                            <a href={result.productId} onClick={(event) => event.preventDefault()}>
                                                <span className="result-image" style={{ backgroundImage: `url("${result.imageURL}")` }}></span>
                                                <span className="result-title">{result.title[local.local] ? result.title[local.local] : result.title.fa} <span>{general[local.local].by}</span></span>
                                                <span className="result-shop-info">{result.sellerInfo.shopDisplayName}</span>
                                            </a>
                                        </li>
                                    )
                                })
                        ...
                        }
                        {
                            state.requestData?.length === 0
                                ...
                                <li className="show-all-results">
                                    <a href={state.searchText} onClick={(event) => event.preventDefault()}>
                                        <span>{general[local.local].showAllResults} <span className="search-text">{state.searchText}</span></span>
                                    </a>
                                </li>
                                : ''
                        }
